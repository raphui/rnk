...
#include <errno.h>
#include <utils.h>
#include <mach/usart_pl010.h>

static unsigned short pl010_baud_rate(unsigned int baud)
{
...
static int pl010_usart_init(struct usart_master *usart)
{
	int ret = 0;
...
static void pl010_usart_print(struct usart *usart, unsigned char byte)
{
}

static int pl010_usart_printl(struct usart_master *usart, const char *string)
{
	int size = 0;

	while (*string) {
		pl010_usart_print(usart, *string++);
		size++;
	}
...
static int pl010_usart_write(struct usart_master *usart, unsigned char *buff, unsigned int len)
{
...
static int pl010_usart_read(struct usart_master *usart, unsigned char *buff, unsigned int len)
{
//	USART_TypeDef *USART = (USART_TypeDef *)usart->base_reg;
//	int i = 0;
//	int ret = 0;
...
struct usart_operations usart_ops = {
	.read = pl010_usart_read,
	.write = pl010_usart_write,
	.print = pl010_usart_print,
	.printl = pl010_usart_printl,
};

static int pl010_usart_of_init(struct usart_master *usart)
{
	int offset;
	int ret = 0;
...
static int pl010_usart_init(struct device *dev)
{
	int ret = 0;
...
struct device pl010_usart_driver = {
